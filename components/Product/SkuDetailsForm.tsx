...
const SkuDetailsForm: FC<ISkuDetailsFormProps> = ({
	...
	const handleSubmit = async (e: any) => {
		...
		try {
			...
			if (!lifetime) {
				skuForm.validity =
					skuForm.validityType === 'months'
						? skuForm.validity * 30 // convert to days
						: skuForm.validity * 365; // convert to days
			} else {
			...
			toast.success(skuDetailsRes.message); // Success toast
		} catch (error: any) {
			if (error.response) {
				if (Array.isArray(error.response?.data?.message)) {
					return error.response.data.message.forEach((message: any) => {
						toast.error(message); // Error toast for each message
					});
				} else {
					return toast.error(error.response.data.message); // Single error toast
				}
			}
			toast.error(error.message); // General error toast
		} finally {
			setIsLoading(false);
		}
	};
